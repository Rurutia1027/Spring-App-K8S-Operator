...
Inspired by [Build Your Own Kubernetes Operators with Go and Kubebuilder](https://www.youtube.com/watch?v=odP153inZUo).

Declare a `SpringApp` custom resource; the operator reconciles Deployment, Service, ConfigMap, database wiring, status, and finalizers. No cloud provider required — runs entirely on local Kubernetes (Kind).

## What's inside

| Component | Description |
|-----------|-------------|
| [`app/`](app/) | Spring Boot 3 REST API — CRUD on `Note` entities backed by PostgreSQL |
...
kind create cluster --name spring-app

# 2. Build images
cd operator
make docker-build IMG=spring-app-operator:dev
make docker-build-app APP_IMG=notes-service:dev

# 3. Load images into Kind
kind load docker-image spring-app-operator:dev --name spring-app
kind load docker-image notes-service:dev --name spring-app

# 4. Install operator (CRD + RBAC + controller)
make install
make deploy IMG=spring-app-operator:dev
...
### Spring Boot app (without Kubernetes)
...
make undeploy
make uninstall
kind delete cluster --name spring-app
```
